...
I would like to thank first of all, to my advisor Mario Diaz for his many advice and encouragement in the difficult times and having me on board of TOROS.
...
To my friends Marc Normandin, Shihan Weerathunga and Ervin Vilchis, partners in this journey that was the doctoral program.

To Bruno Sanchez and Juan Bautista Cabral for his disinterested help and friendship.

I want to dedicate this to the memory of Cristina Torres, a wonderful person, ready to reach out to help people. May she rest in peace.
...
Traditional Astronomy can complement this limitation in gravitational wave detection where optical astronomy is stronger: localization.
However, this poses other technological challenges of a different kind.
In the era of multi-messenger Astronomy, low latency response time after detection is crucial to have any hope in detecting 
...
In this way, Optical Astronomy is a vital partner in the understanding of the physics in compact mergers.
...
These are linear disturbances of a flat background metric, that are propagated outwards with the speed of light fading with the inverse of the distance.
...
Even the most powerful astronomical events, like a black hole merger will produce disturbances the order of $10^{-23}$ m at a distance of a few hundreds Mpc.
...
The many GRBs detected in the first few decades rapidly show that the directions they were coming from, were isotropically distributed in the sky.
...
The next panels show in sequence the first epoch and second epoch imaging, and difference (upper row F606W/optical and lower row F160W/nIR). From \citet{2013Natur.500..547T}.}
\label{fig:tanvir}
\end{figure}
...
The optical light curve can serve as a probe into the core of the system and shed light to the intricacies and details of the merge process.
...
The whole operation of TOROS can be divided in several stages
...
Another advantage of Python is the ease to install these packages. Python has a standard package manager module (\emph{pip}) that takes cares of dependencies, compilable or not.
...
In situations of unbalanced sets, the `recall' measure is a more meaningful score of the classification.
...
The idea for the astroalign algorithm can be summarized in the following steps
...
The first method, optimal\_system will return the difference image along with the optimal image to be subtracted (the ruined reference), 
...
because it can be done by median-stacking good images, or using Lucky Imaging, or some other method.

To convert the problem into a linear one, we decompose the kernel into a linear combination of ``\emph{basis}'' functions $B$.
...
A simpler basis was proposed by \citet{2008MNRAS.386L..77B} and it's effectively a Kronecker Delta function for each pixel.
