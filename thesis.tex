...
The PyGCN module, developed by Dr. Leo Singer, handles the reception of GCN notices from LVC using the TCP protocol.
...
LIGO has three other localization estimation codes: LALINFERENCE\_MCMC, LALINFERENCE\_NEST and LALINFERENCE\_BAMBI.
...
The algorithm relies on excess power detection; coincidence between different detectors; and analysis of coincident events using MCMC Bayesian inference.
...
This is done through a broker website written in Python using the Django\footnote{\href{https://www.djangoproject.com}{https://www.djangoproject.com}} web framework (see figure \ref{fig:brokerhead})
...
Each telescope representative is assigned a username and password to access a 
website\footnote{\href{http://toros.utrgv.edu/winnow/broker/}{http://toros.utrgv.edu/winnow/broker/}} that is hosted in UT RGV servers.
...
This bleeding is very significant for bright stars and less so for dimmer stars but still present nonetheless.
...
To make a suitable subtraction of the two images, one has to match the frames to be exactly the same \emph{seeing}. 
...
The PSF of small fields of view can usually be considered constant across the image, but for large fields of view this is no longer true.
...
In \citet{2000A&AS..144..363A}, they propose a work-around to the computational problem, using for the calculation of the $M$ matrix,
...
To tackle the problem of PSF spatial variability he proposes dividing the image on a grid and to perform a subtraction on each grid element.
...
is that they can be fooled by saturated stars and bad pixels on the image.
...
The implementation of these set of algorithms spun the creation of a new Python module named \emph{ois}\footnote{\href{https://github.com/toros-astro/ois}{https://github.com/toros-astro/ois}}.
...
Machine Learning is an extensive and intensive area of research.
...
Even more important than the particular algorithm 
...
For our main test, I decided to train a Random Forest ML algorithm on a training set developed especially for this.
...
Each node represents a bifurcation based on one feature. 
...
A single Decision Tree suffers from great variance, even for big amounts of data. 
The bifurcations on the greedy algorithm are naturally very dependent on the training set. 
A small variation on it may create a completely different tree. 
...
In the next section I explain how the training data was prepared.
...
Random Forest can also provide an importance score for each feature, based on how far up in the tree structure it is within a tree, 
since the features with most discriminative power are selected as first decision nodes.
More precisely, the importance is defined as the error reduction (or the decrease in impurity) on a node, multiplied by the number of samples that were routed to that node.
This quantity is added to the feature doing the branching at that node, and it is computed for each internal node of the tree and then averaged over the forest.
Nodes high up in a tree tend to branch off more samples, and accordingly receive a higher multiplicative factor.
The importances are finally normalized by the sum, so that they all add up to 1.

In figure \ref{fig:featureimportance} we show a graph listing the relative importance for each of the features along with one standard deviation.
...
\caption{The relative `importance' of each feature calculated with Random Forest algorithm. All the importances are normalized so that they add to 1.}
...
We can see that none of the features by itself clearly separates the data, as is usually the case for typical Machine Learning problems.
...
The remaining parts of this chapter refer to the observation campaign for the GW150914 event follow-up search and its results.
...
The maps provided initial spatial localization of 50\% and 90\% confidence regions encompassing about 750 and 200 square degrees, respectively.
...
Despite the small area covered by TOROS, the O1 campaign allowed the collaboration to test the detection and response systems to the alerts.
...
These joint filters for GW and Optical parameters ensures the targets are visible at each telescope site, 
and that they have some significant probability of being the host. 
...
to identify those pixels with little to no response to light exposure.
...
As explained in section \ref{section:dia}, the Bramich method uses the information from all the pixels on the image, whether part of the background or a source,
...
the background was introducing large depressions of negative pixels and we had to turn background estimation off on those (see figure \ref{fig:tileexamples}).
...
GitHub\footnote{\href{https://github.com/toros-astro}{https://github.com/toros-astro}}.
Two of them have unit tests and hooked to Travis CI\footnote{\href{https://travis-ci.org}{https://travis-ci.org}}
website for continuous-integration and code coverage\footnote{\href{https://codecov.io/gh/toros-astro}{https://codecov.io/gh/toros-astro}}.
...
The module \emph{reproject}\footnote{\href{https://reproject.readthedocs.io}{https://reproject.readthedocs.io}} will resample or reproject images when WCS information is provided, but it does not address the problem of registration when one or both images lack WCS information.
The module \emph{alipy}\footnote{\href{https://github.com/japs/alipy}{https://github.com/japs/alipy}} does not rely on WCS information, but uses external programs like IRAF and SExtractor and deals exclusively in the FITS format for images.
...
As we mentioned before, the core idea of the algorithm consists in characterizing asterisms on the image in a unique way, 
...
\item Once both invariant catalog trees are made, we continue with the cross matching under a RANSAC algorithm \begin{enumerate}
	...
	\item The transformation is considered satisfactory if it can match at least 80\% of the other stars in the catalog.
	If it is not satisfactory, iterate RANSAC until one satisfactory affine transformation is found, or until we reach the maximum number of iterations.
\end{enumerate}
\end{enumerate}
...
Some of the correspondences may arise from random pairing of unrelated asterisms that do not represent the true transformation between the images.
...
The documentation and usage for the code can be found in \href{http://optimal-image-subtraction.readthedocs.io}{readthedocs.io}\footnote{
\href{http://optimal-image-subtraction.readthedocs.io}{http://optimal-image-subtraction.readthedocs.io}}.
The source code is available in the TOROS GitHub repository\footnote{\href{https://github.com/toros-astro/ois}{https://github.com/toros-astro/ois}}.
...
This imposes strong demands on the pipeline data processing and storage to be on location, with failure tolerance arising as a key constrain feature.

To assess this issue, Corral provides the formalization of a pipeline framework based on the concepts of Steps and Models following the 
well known design pattern \textit{Model--View--Controller} (MVC) and built on top of a SQL Relational Database.

Every Model instance can be thought of as a table in the database and represents a component or piece of data in the pipeline.
...
When one of these is raised, it is compulsory that an Alert will notify the research group as specified in the implementation.
...
without worrying about creating code for multi-processing workflows, coding architecture or code to interact with the database.
...
Source code for the project is hosted in the TOROS GitHub page \href{https://github.com/toros-astro/corral}{https://github.com/toros-astro/corral}.
Documentation and a tutorial to get started can be found in \href{http://corral.readthedocs.io/}{http://corral.readthedocs.io/}.
...
The website at \href{https://toros.utrgv.edu/winnow}{https://toros.utrgv.edu/winnow}, is a web interface that presents users cut-outs of possible real transients.
...
Earlier attempts at ML relied on sets of data generated by the contribution of undergraduate students at UT Rio Grande Valley.
...
\Gamma^{a}_{bc} &= g^{ad} \Gamma_{dbc} \\ 
 &=(\eta - h)^{ad} \Gamma_{dbc}  \nonumber \\
 &= \frac{1}{2} \eta^{ad} \left( h_{cd,b} + h_{cb,d} - h_{db,c} \right) + \order(h^2) \nonumber
\end{align}
...
This is called the transverse traceless (TT) gauge.
...
\begin{equation}
B_{n,p, q}(u,v) = e^{-(u^2+v^2)/2 \sigma_n^2} \times u^{p} v^{q}
\end{equation}
...
He will in the future keep working on the TOROS project, completing the software and training incoming students.
