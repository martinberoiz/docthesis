...
we are heading to a regime of several tens of TB and the order of million events per night in the LSST\footnote{http://www.lsst.org, http://www.lsst.org/files/docs/sciencebook/SB\_8.pdf} era.
The data load makes it unmanageable if not done by automated agents.
% LSST ref Ivezic, Z., Tyson, J. A., Allsman, R., Andrew, J., Angel, R., and for the LSST Collaboration (2008). LSST: from Science Drivers to Reference Design and Anticipated Data Products. ArXiv e-prints.
% LSST Science Collaborations, Abell, P. A., Allison, J., Anderson, S. F., Andrew, J. R., Angel, J. R. P., Armus, L., Arnett, D., Asztalos, S. J., Axelrod, T. S., and et al. (2009). LSST Science Book, Version 2.0. ArXiv e-prints.
...
%GRBs were an enigma until ultra-fast event dissemination allowed optical identification of afterglows.
...
In the same way as long GRBs prompted the follow-up observations across the spectrum,
a new type of event associated with short GRBs can also prompt a new kind of MMA.
Short GRBs are now believed to have originators in the merger of compact objects.
Along with the GW produced in these mergers, an companion EM thermal radiation will radiate isotropically from the merger.
This EM counterpart, is believed to shine about a thousand times brighter than a Nova, and it was named accordingly 
`Kilonova'\footnote{It is also reference in the literature under the names xx and xx.}.

If there is a chance to detect the relatively faint Kilonova, it will be greatly helped if we have prompt information of its GW counterpart, or its associated GRB.
Its detection will most likely be a result of MMA.
