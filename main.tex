...
This is the task of a Machine Learning (ML) agent trained for that purpose. The real bogus classifier, as it is named in the literature relies on --mostly morphological-- features to perform the classification.
...
Once the kernel is found, we convolve the reference image with it and subtract any remaining background given by the module OIS when needed.
This modified reference is then subtracted to the original image to get the subtracted images.

On this subtracted images, we ran SExtractor to extract posibles sources.
