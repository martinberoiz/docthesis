...
In February 2016, celebrating the centenary anniversary of Einstein's first paper on gravitational waves(``Approximate integration of the field equations of gravitation''), 
...
In September 14, 2015, a GW detection labeled GW150914 was finally detected by the LIGO Collaboration.
...
This bar has already reached a strain sensitivity of $10^{-21}$ Hz${}^{-1/2}$ in the kHz region.
(P. Astone et al., ``The Gravitational wave detector NAUTILUS operating at T = 0.1K'' Astropart. Phys. 7 (1997) 231-243.)
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The Transient Universe and the Time-Domain Revolution in Astronomy}
...
we are heading to a regime of several tens of TB and the order of million events per night in the LSST era.
...
Some of the latest most interesting advances in Astronomy are related to the discovery and analysis of transient events 

Transient events can last from milliseconds, like Fast Radio Bursts, to a span of days or weeks like Super Nova (SN) explosions.
...
The time scales and methods of detection are so varied as the phenomena under study.
Transient events are the exact opposite of static events, and they don't have a periodic variability.
Their very nature demands a time-domain analysis.

\begin{comment}
List of transients:
- soft gamma repeater (SGR) flares; these are highly magnetized (1015G) neutron stars that emit ?-ray flares sporadically

\end{comment}

%Basically every area of Astronomy has been enhanced by the time domain analysis of transient events, 
%and this phenomena can only be studied in the field of time domain.

Nonetheless, time domain astronomy poses at least two big main challenges.
The first one deals with the huge amount of data and the process of recovering interesting transients form it. 
This is a {\em Data Mining} process.
...
We cover multi-messenger Astronomy (MMA) and some examples of it in the following sections.

\subsection{Mining Transients}

The identification of transients of interest has to be done among a multitude of other unrelated transient events.
Furthermore, fake transients from the instruments or methods used to mine them, can confuse methods of source extraction.
\textcolor{blue}{copy-paste}Rare events interesting to a particular survey or telescope must then be identified against the stationary 
or slowly varying sky, and must be separated from other transients of a different kind.

This `mining' or harvesting problem is not exclusive of Astronomy and have been tackled by other disciplines by different techniques of many kinds.
...
Classification problems done by automated agents trained for that purpose are usually dealt within the techniques called ``Machine Learning'', and we will devote a great deal of this thesis on this field.
...
\textcolor{blue}{copy-paste} More than ten years ago, the gamma-ray burst (GRB) community ignited the excitement over transient astronomical events. 
GRBs were an enigma until ultra-fast event dissemination allowed optical identification of afterglows, leading to rich data and rich science.
...
It was actually GRBs, one of the first things to be studied in a multi-band fashion,
and for this reason, they settled the infrastructure for an effective multi-messenger astronomy.
...
Instead of this, they found strange very energetic bursts of gamma rays coming from outer space. (R. W. Klebesadel, I. B. Strong, and R. A. Olson, ApJ (Letters) 182, L85 (1973).)
%It was no long until they found that the isotropic distribution of them, were of extragalactic origin

%optical supernova glow for months

After a long debate, data finally showed that their isotropic distribution on the sky was due to their extragalactic origin.
Nevertheless, the distance to these events remained elusive.
To find an estimate of the distance at which these bursts happened, 
it was necessary to find a counterpart whose distance was easier to infer.

This settled the energies liberated in this bursts to be
...
Just like in the case for GRBs, that has counterparts across all EM frequencies, other types of transient events will have the same characteristics.
Most important for us, is the case of the Kilonova, a transient event with an optical afterglow, product of the merge of a binary with at least one NS.

% Add: GW can trigger searches for optical counterparts, but the inverse is also true. 
% There are efforts to search for GW in ligo data triggered by optical observstions.
% E. Thrane et al, Phys. Rev. D 83, 083004 (2011).

\begin{comment}
Long and short GRB afterglows peak a few minutes after the prompt EM/GW emission
(D.A. Kann et al, arXiv:0804.1959v2 (2008).)(D.A. Kann et al, ApJ 720, 1513 (2010).)
and it is critical to have EM observations as soon as possible after the GW trigger validation. 
Kilo-novae model afterglows peak about a day after the GW emission (B. D. Metzger et al MNRAS 406, 2650 (2010).)
...
The brightness of this flash of gamma-rays can temporarily overwhelm all other gamma-ray sources in the Universe. 
The burst can last from a fraction of a second to over a thousand seconds. 
...
With the new GW detections of LIGO, a new kind of multi-messenger Astronomy is born.
...
The essential enabling technologies will be automated, robust classification and decision making for the optimal use of follow-up facilities. 
Given the exponential growth of data rates, the traditional ``manual'' approach from the past will simply not scale to the next generation of surveys, especially if one is interested in the rarer transients.

a broad diversity of the fundamentally different physical phenomena look the same at the instance of their event detection, e.g., a ``new'' or a much brighter point source relative to the baseline image.}
...
For a more detailed explanation of the Kilonova and a possible detection in the infrared, please refer to the section \ref{kilonovachapter}.
...
\chapter{The O1 campaign: GW150914 Optical Observations}
...
On the night of September 16, 2015, a LIGO alert for a GW arrived through the TOROS alert receiver system.
...
\caption{GW150914 event observed by the LIGO Hanford (H1, left column panels) and Livingston (L1, right column panels) detectors. Top row is the strain signal. Second row is the GW strain projected onto each detector in the 35--350 Hz band. Solid lines show a numerical relativity waveform for a system with parameters consistent with those recovered from GW150914. Third row: Residuals after subtracting the filtered numerical relativity waveform from the filtered detector time series. Bottom row: A time frequency representation of the strain data.}
...
\chapter{The O1 campaign: GW150914 Data Analysis}
