...
From 0.1 TB and about 10 to 100 events per night now, to about 30 TB and $10^5$ to $10^6$ events per night in the LSST era, 
the data load and the resources to process them become overwhelming if not done by automated agents.

Buried in this huge amount of data, lie many interesting events.
In fact, some of the latest advances in Astronomy are related to the discovery and analysis of transient events 
that last from milliseconds, like Fast Radio Bursts, to a span of days or weeks like Super Nova (SN) explosions.
Some of the events are recurring, with or without predictability of the next event, and some of them are only a one-time occurrence.
...
The first one deals with the huge amount of data.
...
Rare events interesting to a particular survey or telescope must then be identified against the stationary 
...
According to \textcolor{red}{Batse (Swift? look for ref)} data, GRBs seem to come from two distinct population.
...
Long GRBs are associated with Super Nova explosions \textcolor{red}{ref?}.
This 
