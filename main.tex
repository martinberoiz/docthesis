...
With purpose in mind of complementing the LIGO observations in the electromagnetic (EM) side, Dr. Mario Diaz from UT RGV and Dr. Diego Lambas from IATE, set up the TOROS project. TOROS (the Transient Optical Robotic Observatory of the South), has the main goal of scanning the uncertainty region of LIGO localization of GW events with a wide field telescope and a large CCD pixel camera, searching for EM transients potential candidates of such GW event.

The original project consisted in the construction of an observatory site in Cordon Macon, a high mountain in the Andes mountain range in north of Argentina. 

To this end, TOROS signed a Memorandum Of Understanding (MOU) with LVC, along with other XX observing sites, to be allowed to receive the alerts for the O1 LIGO campaign. TOROS successfully participated of this campaign, where the first GW was detected, GW150914, and other two events with lower intensity were also detected, one in December ...
...
Despite the little area covered by TOROS, the O1 campaign allowed the collaboration to test the detection and response systems to the alerts. Several other institutions showed interest in the project after this achievement and since then the collaboration has grown.

Since the first concept of TOROS as a single observatory in the Andes mountains, the project has benefited of the addition of several institutions that showed interest to participate as well. The collaboration has now partners in Mexico, and the Gemini spectrograph as well as the telescopes in Chile and Cordoba previously mentioned.

Wether it be the original design or the new dynamic proposed by the enlargement of the collaboration, the interesting targets to TOROS remain to be the potential Kilonovas that are believed to appear during NS-NS or BH-NS mergers, along with its GW counterpart.

The state of the Kilonovas is now a bit more than a theoretical product of simulations and theoretical speculations of its existence. Nonetheless, the proposed emission mechanism is well grounded in the literature. The first to discover this emission mechanism was this guy in this year, but it was not clear for many years, which astronomical scenario could give raise to the so called r-process. Someguy was the first to suggest that a BNS merger could be a suitable environment for the r-process and its radiation emission.  
...
The Kilonovas have isotropic radiation. Even though short GRBs emission, which is also associated with BNS mergers, is also present during the BNS merger, the r-process emission in the jet and other merger neutron ejecta, is fairly isotropic. The chances to detect the GRB jet are very limited by the collimation of the jet and our line of sight, but r-process radiation is not. 

The Kilonovas are bright, they are 1,000 times brighter than a nova. Observational and theoretical evidence suggest a relation between... 

Another fundamental element that the multi-messenger astronomy contributes with is the identification of host galaxies. With them one can...

The merger of a binary system with a NS in it, is a very complex merge and several kind of factors can effect the radiation pattern and light-curve of the Kilonova as well as the GRW waveform. The optical light curve can shed light to the intricacies and details of the merge process as a probe for it. 

In the following chapter, I give a more detailed explanation of the Kilonova and a possible detection in the infrared. 
...
\include{chapter_kilonova/chapter}
...
in all the stages from receiving the alert to propose new transient candidates.
...
The alert receiver is located in a virtual machine server provided by UTRGV IT Services.
It consists of a Python script that runs uninterruptedly waiting for a signal from the 
NASA service \textcolor{red}{(Find exact name)} delivered on a specific port reserved for that purpose.

The script makes use of the Python module gcn??? (Leo Singer) which handles the reception of the VO Event
and does minimum process to deliver the XML body of the VO Event by email to predetermined recipients.
...
Right now, this is done in a separate script.

As mentioned before, the target selection is done with a separate script.
This script makes use of the skymap provided in the alert notice and the 
...
due to their use of more up-to-date input catalogues
and the fact that they don't make a blue luminosity cut.

Another catalog along these lines is the Glade GW Catalog. Glade Catalog has xxxx galaxies listes, this many more than CWGC and extends up to XX Mpc.
Nonetheless many distances are inferred by Machine Learning Methods, and completitude of the Catalog... bla bla

The target selection relies on two main things. The probabilty of detection on the target pixel in the all sky map given by LIGO at the time of the alert,
and several filter cuts on distance, blue luminosity and apparent magnitude. 

This marriage between GW and Observational parameters ensures the targets are visible at each telescope site and having some probability to be the hosts.
The list of filter cuts are summarized in table xxx.

Once the list of targets is done, we need to communicate the targets to each telescope.
This is done through a website written in Django.
Each telescope representative has assigned a username and password to access a website hosted on UTRGV servers.
This website has a simple interface of tables for each observatory and a list of targets observable from each location.

The telescope admin then selects targets from the lists, to reserve them for his or her observatory. 
The website enforces that the list of targets is unique and that no two observatories are observing the same target. 
...
The website can also output a circular draft of all the targets observed by each telescope to be presented to the rest of the community.

Those are the normal workflow for the operations on a LIGO campaign.
...
Most notably is the loading of the observation targets to the website.
Making this automatic, would require a REST framework on the Django website and a bla bla,
both of which are beyond the computational abilities of who writes this thesis.
