...
Soon after a coincident detection is made on the operating detectors, 
...
These three codes are stochastic samplers that estimate the full GW parameter space and then marginalize over every parameter that is not the sky location.
...
At the time of the detection, observatories will rely on the position estimation of BAYESTAR.
By the time the LALINFERENCE maps are released, most of the observations would be already done.

The retrieval of the BAYESTAR sky map is done through a authenticated HTML GET request to the Grace Database of the GCN/TAN website.
...
For each of these images, we aligned a reference image with it using the package `astroalign' (see section \ref{section:astroalign}) 
and also performed a image subtraction using the delta basis method on a 4$\times$4 grid.
