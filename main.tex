...
General Relativity predicts that very massive or energetic events will create traveling perturbations of the underlying spacetime metric in the form of {\em gravitational waves}.
...
EN mi sugerencia 1.1 es como lo tenes, 1.2 es distintas fuentes astrofisica, 1.3 es LIGO y VIRGO y los detectores que convendria que fueran explicados un poco mas tecnicamente (mirar papar de detección) y 1.4 deberia ser una discusion de la deteccion y lo que significa?por ejemplo hay predicciones de rates de fusion de estrellas de neutrones tenes que hablar de eso?mira papers de  Belczinski y de Vicki Kalogera?
\end{comment}
...
`Kilonova'\footnote{It is also referenced in the literature under the name {\em macronova}}.
...
VOEvent is the International Virtual Observatory Alliance (IVOA) recommended mechanism for describing astronomical transients that has been adopted as an international standard.
...
it is an XML file notice that ``{\em defines the content and meaning of a standard information packet for representing, transmitting, publishing and archiving information 
...
Another LAL code for unmodeled bursts is the LAL Inference Burst, or LIB \textcolor{red}{(Lynch et al. 2015)}.
This LAL code, unlike the others in LAL Inference, has low latency. It assumes a sinusoidally modulated Gaussian as the wave-form.
Even though this assumption may not be the true signal, it can produce localizations for a wide variety of waveforms, including compact object mergers.
...
At the time of the detection, observatories will rely on the position estimation of BAYESTAR, cWB or LIB.
By the time the LALINFERENCE maps are released, most of the observations would be already done.

The retrieval of the low latency sky maps is done through a authenticated HTML GET request to the Grace Database of the GCN/TAN website.
...
(Apparent Magnitude, Distance and Absolute Magnitude) as well as objects with Right Ascension values that make them not observable for the time of the year.
...
and we assign a few dozens of the most likely to each observatory for the observation.
...
Three of them had standard SDSS {\em gri} filters and the fourth one was a {\em clear} filter.
Only the images from filter {\em i} are usable since the other 3 telescopes failed to return data.
...
The CSTAR 2010 {\em i} set has a median exposure time of 42 s.
...
The CCD had a few dead columns and rows and bright stars suffer from `bleeding' (see figure \ref{bleeding}) even though they have low exposure time.
...
In our case, we used a reference image constructed by stacking several `best' images from June and then we aligned this to each of the frames in the data set.

To make a suitable subtraction of two images, one has to match the frames to exactly the same seeing. 
Image Difference is a technique to find a convolution kernel that best describes (in some minimization sense) the change in point spread function between the images. 
The idea is to degrade the good seeing image---our reference image---to match the seeing of our second image. 
Finding the proper kernel can be a delicate operation and there's plenty of literature on the subject. 
...
Ideally, the reference image is the one with the best {\em seeing}, 
...
The inversion problem \ref{diaeq} can be computationally very expensive to invert, especially for an arbitrary shape of the kernel.
To convert the problem into a linear one, we decompose the kernel into a linear combination of ``{\em basis}'' kernels $B$.
...
For a more extended discussion of this method, we refer the reader to appendix \ref{appendix:dia}.

\citet{1998ApJ...503..325A} was the first to introduce this least-squares method to find an optimal image. 
He proposed the use of multi-Gaussian functions with modulating polynomials as the kernel basis.
...
Many PSF differences can not be modeled accurately, especially when there is very little difference between the image and its reference.
The method can not fit for the $\sigma$ spread for the Gaussian functions and it has to be determined empirically from the data.
...
The spatial variation is done by further expanding each coefficient into a polynomial on the $(x, y)$ position on the image.
...
only pixels on {\em stamps} around sources and considering a nearly constant PSF on each stamp.
...
They are significantly much more than those of \citet{1998ApJ...503..325A} growing with the square of the kernel side, 
...
To tackle the problem of spatially variable PSF he proposes dividing the image on a grid and perform a subtraction on each grid element.
In \citet{2008PASP..120..449M} they propose a method similar to \citet{1998ApJ...503..325A} with the use of {\em stamps} to deal with PSF variation.
Althought, introducing new coefficients for space variation is more costly on the Delta basis because of the larger number of original variables to fit in the kernel.

These methods based on least-square optimization can be fooled with saturated stars and bad pixels on the image.
...
The implementation of these set of algorithms became a Python module named ois\footnote{\url{https://github.com/toros-astro/ois}}.
For more details on the Python implementation we refer the reader to section \ref{section:ois}.
...
We used a kernel side of 11 pixels, which correspond to 2.9'. 
This is more than enough to accommodate for PSF differences: the FWHM from a typical image ranges between 1 and 2 pixels.

Figure \ref{} shows some examples of the difference images
...
For that two images, one taken at the epoch of interest and another one used as a reference. 
Ideally, the reference is an archive image or a stack of several others to improve signal to noise ratio, cosmic rays and other contaminants on the image.
...
Software to perform the alignments was done using a method described in section \ref{section:astroalign}.
I describe there a method based on asterism matching inspired in astrometry.net.

The subtraction methods are explained in section \ref{section:ois}.
...
Gravitational Waves Galaxy Catalog (GWGC) (A List of Galaxies for Gravitational Wave Searches, Darren J. White et al, 2011)
...
algorithms\footnote{For more information on the package implementation, see section \ref{section:ois}.}.
...
The references images were aligned pixel to pixel to the science images with the software Astroalign (see section \ref{section:astroalign} for more details on this Python module).
...
We perform the subtraction using the Python module OIS (see section \ref{section:ois}).
...
\include{chapter_astroalign/chapter} \label{section:astroalign}
