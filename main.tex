...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
The novel research in the Gravitational Wave domain has introduced new challenges to Astronomy and has opened a new window to the universe.
...
This thesis will discuss the implementation of a software {\em pipeline} for the TOROS project and the results obtained during the O1 campaign of LIGO.

In the first chapter, I introduce some of the theoretical and experimental background for gravitational waves.

In chapter 2, I discuss the field of multi-messenger and time domain Astronomy, and I discuss the expected electromagnetic counterparts to some of the gravitational wave signals,
in particular the Kilonova, the most promising electromagnetic counterpart.

In chapter 3, I discuss techniques and software developed for the TOROS pipeline. 
At the core of this pipeline lies an Image Difference algorithm along with a Machine Learning classifier to propose transient candidates.

In chapter 4 I show results of the O1 campaign for the follow-up of GW150914, the first gravitational wave detected by LIGO.

In chapter 5 I discuss some of the software that complements the TOROS pipeline, that were developed during my PhD.

Chapter 6 brings up conclusions and further work needed to complete this work.
...
In our case, we used a reference image constructed by stacking several CSTAR images from the best-{\em seeing} month of June 
and then we aligned this to each of the frames in the data set.

To make a suitable subtraction of the two images, one has to match the frames to exactly the same {\em seeing}. 
...
The idea is to degrade the good {\em seeing} image ---our reference image--- to match the {\em seeing} of our second image. 
...
First, they decompose the kernel into a linear combination of ``{\em basis}'' kernels $B$.
...
The implementation of these set of algorithms spun the creation of a new Python module named {\em ois}\footnote{\url{https://github.com/toros-astro/ois}}.
For more details on the {\em ois} Python implementation we refer the reader to section \ref{section:ois}.
...
Machine Learning in its {\em supervised} variant, is the general name given to a set of algorithms that can 
...
Retrieved instances are those classified as `real', regardless of their true class. The fraction of those that are actually `real' is the {\em precision}. \\
$P = TP/(TP+FP)$
...
The same procedure was done on the {\em a posteriori} reference frames.
...
Even though the Python {\em ois} module can do a simultaneous background estimation to be subtracted, 
...
We perform the subtraction using the Python module {\em ois} (see section \ref{section:ois}).
...
The first attempts to build software for the TOROS pipeline lacked modularity, or in the language of Computer Science, {\em separation of concerns}.
...
The module {\em reproject} will resample or reproject images when WCS information is provided, but it does not address the problem of registration when one or both images lack WCS information.
The module {\em alipy} does not rely on WCS information, but uses external programs like IRAF and SExtractor and deals exclusively in the FITS format for images.
...
For {\em astroalign}, we don't need a catalog of the whole sky, since we are only interested in the region of the sky covered by the image at hand. 
...
Optimal Image Subtraction ({\em ois}) is a Python module that implements several Difference Image Analysis methods as described in section \ref{section:dia}.
...
% an Open Source BSD-3 License pure Python package capable of creating a high performance abstraction layer over a data warehouse, with multiprocessing data manipulation, and expose to the 
% scientist a simple Object Oriented structures that seizes the power of modern multi-core computer hardware.
...
Along with the broker website (see section \ref{section:broker}), the web server for the TOROS project hosts {\em winnow}, 
an interactive Real/Bogus {\em web-app} for human classification.
